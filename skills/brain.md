...
1. **Never store secrets.** No API keys, passwords, or tokens. Reference by variable name only.
2. **Check before you remember.** Always `brain_recall` first to avoid duplicates.
3. **Prefer updating over creating.** If a thought exists on the topic, `brain_update` it.
4. **Respect confidence scores.** A confidence: 1.0 human-sourced thought outweighs your inferences.
5. **Be honest about confidence.** If you're inferring, don't set confidence to 1.0.
